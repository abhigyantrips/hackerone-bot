...
class SlashReports(commands.Cog):
    ...
    async def reports(self, ctx, id: str):
        ...
        if "default" in reportsjson['data']['relationships']['reporter']['data']['attributes']['profile_picture']['62x62']:
            ...
            reporterpic = reportsjson['data']['relationships']['reporter']['data']['attributes']['profile_picture']['62x62']
        ...
        embed.add_field(
            name="Created at", 
            value=f"<t:{round(dp.parse(reportsjson['data']['attributes']['created_at']).timestamp())}:f>" or "-",
            inline=True,
        )
        embed.add_field(
            name="Disclosed at",
            value=f"<t:{round(dp.parse(reportsjson['data']['attributes']['disclosed_at']).timestamp())}:f>" or "-",
            inline=True,
        )
        embed.add_field(
            name="Program",
            value=reportsjson['data']['relationships']['program']['data']['attributes']['handle'] or "-",
            inline=False,
        )
        embed.add_field(
            name="Severity",
            value=(reportsjson['data']['relationships']['severity']['data']['attributes']['rating'] or "-").capitalize(),
            inline=True,
        )
        embed.add_field(
            name="Score",
            value=reportsjson['data']['relationships']['severity']['data']['attributes']['score'] or "-",
            inline=True,
        )
        embed.add_field(
            name="Bounty",
            value=(reportsjson['data']['relationships']['bounties']['data'][0]['attributes']['awarded_currency'] or "") + f" {bounty}",
            inline=True,
        )
